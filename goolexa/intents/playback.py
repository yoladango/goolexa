...
@ask.intent('GoolexaRefreshLibrary')
def index():
    api.start_indexing()
...
@ask.intent('GoolexaCurrentlyPlayingIntent')
...
@ask.intent('GoolexaListAllPlaylists')
...
@ask.intent("GoolexaThumbsUpIntent")
def thumbs_up():
    if len(queue.song_ids) == 0:
...
@ask.intent("GoolexaThumbsDownIntent")
def thumbs_down():
    if len(queue.song_ids) == 0:
        return statement(render_template("thumbs_no_song"))
...
@ask.intent("GoolexaRestartTracksIntent")
def restart_tracks():
    if len(queue.song_ids) == 0:
...
@ask.intent("GoolexaSkipTo")
# https://github.com/stevenleeg/goolexa/issues/28
